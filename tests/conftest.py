import pytest
from anyio import create_task_group, create_tcp_server, sleep
...
class FakeESLServer(object):
    ...
    def setup_commands(self):
        self.commands["api khomp show links concise"] = (
            "B00L00:kes{SignalLost},sync\n"
            + "B01L00:kesOk,sync\n"
            + "B01L01:[ksigInactive]\n"
        )
    ...
    async def start_server(self):
        ...
        async with create_task_group() as self.tg, await create_tcp_server(
            self._port
        ) as self.server:
            self._running = True
    ...
    async def fake_event_plain(self, client, data):
        await self.protocol_send(
            client, ["Content-Type: text/event-plain", "Content-Length: %s" % len(data)]
        )
    ...
    async def disconnect(self, client):
        await self.protocol_send(
            client, ["Content-Type: text/disconnect-notice", "Content-Length: 67"]
        )
